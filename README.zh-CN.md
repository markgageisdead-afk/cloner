...
.agents/skills/
  clone-website/    # 规范技能与检查参考
.claude/commands/
  clone-website.md  # Claude Code 的轻量桥接文件
...
项目在 `.agents/skills/clone-website/` 中维护一份可移植的 Agent Skill。Codex、Cursor 和 OpenCode 会直接读取它。Claude Code 通过 `.claude/commands/clone-website.md` 中的轻量命令桥接文件运行，因此无需向其他代理暴露重复技能也能保留 `/clone-website` 及其参数。

请直接编辑规范技能。本项目不再包含各平台的生成副本或同步脚本。
